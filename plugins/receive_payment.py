...
async def check_photo(c, m):
    ...
    await c.send_photo(chat_id=Config.OWNER_ID, photo=m.photo.file_id, caption=Translation.PAYMENT_INFO.format(id=ff.id, dc=ff.dc_id, n=ff.first_name, u=ff.username), reply_markup=InlineKeyboardMarkup(button), parse_mode=enums.ParseMode.HTML)
